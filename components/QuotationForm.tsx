...
export function QuotationForm({ existing, onClose, onSaved }: { existing: RecordRow | null; onClose: () => void; onSaved: () => void }) {
  ...
  const [values, setValues] = useState<QuotationFormValues>(() => ({
    ...
    validUntil: existing?.data.validUntil || daysFromNowIso(3),
  ...
  async function persist(status: 'Draft' | 'Sent'): Promise<string | null> {
    ...
    const res = await fetch('/api/records', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ module: 'quotations', status, data })
    });
    const json = await res.json();
    if (!res.ok) { setError(json.message || 'Could not save quotation'); return null; }
  ...
  async function sendQuotation() {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) { setError(json.message || 'Quotation saved, but the email could not be sent'); return; }
      onSaved();
      onClose();
    } finally {
      setSending(false);
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white shadow-xl">
          <div className="flex items-center justify-between border-b border-slate-100 p-4">
            <p className="font-bold">Quotation Preview</p>
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-start justify-between border-b border-slate-100 pb-6">
              <div>
                ...
                <input
                  ...
                  placeholder="Subject (e.g. Office Space Proposal)"
            ...
            <div className="mt-6 grid gap-6 sm:grid-cols-2">
              ...
              <div className="text-left sm:text-right">
                {field('Quote Date', <input className="input" type="date" value={values.issueDate} onChange={(e) => setValues((v) => ({ ...v, issueDate: e.target.value }))} />)}
                <div className="mt-2">
            ...
            <div className="mt-6 flex flex-col gap-2 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
              ...
              <button type="button" onClick={sendQuotation} disabled={saving || sending} className="btn-primary flex w-full items-center justify-center gap-2 sm:w-auto sm:min-w-[120px]">
        ...
        <div className="overflow-x-auto rounded-2xl border border-slate-200">
          <table className="w-full text-left text-xs sm:text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {values.items.map((item, i) => (
                <tr key={i}>
                  <td className="px-3 py-2"><input className="input" placeholder="Office space, meeting room, or any other" value={item.description} onChange={(e) => updateItem(i, { description: e.target.value })} /></td>
